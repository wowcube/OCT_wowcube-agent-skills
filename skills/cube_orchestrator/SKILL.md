...
- `"All 7 handler functions must be present: on_init(), on_tick(), on_tap(int32_t tapid, int32_t count), on_twisted(int32_t twid, uint32_t disconnected_ms), on_pretwisted(int32_t twid), on_shake(int32_t shakeid), on_proc_draw (stub). on_shake and on_proc_draw are link-required stubs — the ARM module fails to link without them. Never build gameplay on shake input (the engine currently always runs the system default go-home); on_proc_draw is only active under #define APP_HAS_PROC_DRAW. If a handler has no game logic, reference every parameter to suppress warnings (e.g., twid; disconnected_ms;)."`

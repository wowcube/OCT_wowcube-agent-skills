...
- All mandatory platform reminders, explicit casts, fixed-width types, and all 5 handlers — still enforced.
...
2. **Run Stage 5 — Package & Verify (mandatory, not optional).** A passing
   ...
   a reason to ship the sim-only `.oct`. (This Stage 5 verification is a hard
   invariant even in YOLO — a sim-only `.oct` is never delivered.)

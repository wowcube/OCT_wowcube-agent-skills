...
- `OCT_wowcube-agent-skills/templates/app_ai_template/src/app_ai_template.h` — OctaviOS API reference and engine patterns (SOURCE OF TRUTH, do NOT copy demo code)
...
(Sounds are listed by their final packed `.mp3` name. The source audio is a WAV
synthesized by the pipeline — or supplied by the user — and encoded to mp3 at pack time.)
...
The GDD lists all required assets (sprites, sounds, fonts), but this skill only specifies them:
- **Sprite PNGs are produced at Stage 3** — either AI-generated by `cube_asset-builder` from per-sprite generation prompts derived from this GDD, or supplied by the user
- **Sounds are synthesized as WAV files by the pipeline** and automatically encoded to the device mp3 format at pack time; the user MAY supply their own `.wav`/`.mp3` files instead
- **The `_ids.h` file is generated by the asset packing tool**, not edited by code agents
- The GDD's asset list serves as a **spec for whoever (or whatever) creates the assets** — it must be detailed enough for a human artist to work independently AND for AI generation prompts to be derived from it

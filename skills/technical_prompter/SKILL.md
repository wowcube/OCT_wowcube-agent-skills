...
- **Prompts MUST reference assets by exact final identifier**:
  ...
  - Animation sequence: `BMP_<base>` (first frame alias) and `BMP_<base>_end` (last frame alias) — both auto-generated by `pack.py` for a zero-padded `_NN`/`_NNN` sequence, zero-based (`_00…`) or one-based (`_01…`).
...
- Animation frames: zero-padded suffix, **2 or 3 digits**, contiguous, starting at either **`_00`/`_000` (zero-based)** or **`_01`/`_001` (one-based)** — the digit width must stay consistent across one sequence (e.g. `hero_idle_00`, `hero_idle_01`; or `boom_001`, `boom_002`, `boom_003`; never mix `_00` and `_001` in the same sequence). Set `anim: "<base>"` and `frame: <N>` matching the numeral in the name (0-based or 1-based, whichever the names use).
...
12. **Manifest validity**:
    ...
    - Animation frames are zero-padded (2 or 3 digits, consistent width) and contiguous, starting at `_00`/`_000` or `_01`/`_001`.
...
14. **Animation placement** — no two animated sprites are assigned (in the prompts' `OCT_add` plane/coordinates) to screens sharing a physical module: a face's four screens are four different modules, adjacent faces share two, opposite faces share none. Any `color: "full"` animation exists only because the user explicitly asked for it, and its manifest/GDD entry carries a "handle VERY carefully" warning.
